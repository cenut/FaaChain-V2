...
public class VoteSet {
    ...
    public void updateHeightViewValidators(long height, int view, List<String> validators) {
        this.height = height;
        this.view = view;
        this.validators = new HashSet<>(validators);
    }
    ...
    public boolean addVote(Vote vote) {
        Sign.SignatureData sig = vote.getSignature();
        ...
        if (vote.getType() == type &&
                ...
                && vote.validate()
                && sig != null) {
                //&& validators.contains(Base64.getEncoder().encode(sig.getAddress()))) {
            String peerId = vote.getPeerID();

            if (vote.getValue() == Vote.VALUE_APPROVE) {
                ByteArray key = ByteArray.of(vote.getBlockHash().getBytes());
    ...
    public boolean isRejected() {
        //FIXME: add rejections size large than zero
        return rejections.size() >0 && rejections.size() >= getTwoThirds();
    }
     ...
     * @return
     */
    public List<Vote> getApprovals(String  blockHash) {
        Map<String, Vote> map = approvals.get(blockHash);
    ...
    public String toString() {
        ...
        return "[" + count + ", " + rejections.size() + ", " + height + ", " + view + ", " + validators + "]";
    }
}
