...
import static com.faa.chain.utils.Assertions.verifyPrecondition;
...
public class Sign {
    ...
    /**
     * Given an arbitrary piece of text and an FAA message signature encoded in bytes,
    ...
    public static class SignatureData {
        ...
        private static final int R_LEN = 32;
        private static final int S_LEN = 32;
        ...
        public byte[] toBytes() {
            byte[] vs = new byte[1];
            vs[0] = v;
            return Bytes.merge(vs, r, s);
        }

        public static SignatureData fromBytes(byte[] bytes) {
            if (bytes == null || bytes.length != LENGTH) {
                return null;
            }

            byte[] vs = Arrays.copyOfRange(bytes, 0, 1);
            byte[] r = Arrays.copyOfRange(bytes, 1, R_LEN + 1);
            byte[] s = Arrays.copyOfRange(bytes, R_LEN + 1, LENGTH);

            return new SignatureData(vs[0], r, s);
        }

        public String getAddress(byte[] message) throws SignatureException {
            BigInteger k = signedMessageToKey(message, this);
            Key key = new Key(k);
