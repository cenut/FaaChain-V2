...
public class RlpDecoder {
     ...
     */
    public static com.faa.chain.rlp.RlpList decode(byte[] rlpEncoded) {
        com.faa.chain.rlp.RlpList rlpList = new com.faa.chain.rlp.RlpList(new ArrayList<>());
        traverse(rlpEncoded, 0, rlpEncoded.length, rlpList);
        return rlpList;
    }

    private static void traverse(byte[] data, int startPos, int endPos, com.faa.chain.rlp.RlpList rlpList) {
        ...
        try {
            ...
            while (startPos < endPos) {
                ...
                if (prefix < OFFSET_SHORT_STRING) {
                    ...
                    rlpList.getValues().add(com.faa.chain.rlp.RlpString.create(rlpData));
                    startPos += 1;

                } else if (prefix == OFFSET_SHORT_STRING) {

                    // null
                    rlpList.getValues().add(com.faa.chain.rlp.RlpString.create(new byte[0]));
                    startPos += 1;
                    ...
                    com.faa.chain.rlp.RlpList newLevelList = new com.faa.chain.rlp.RlpList(new ArrayList<>());
                    ...
                    com.faa.chain.rlp.RlpList newLevelList = new RlpList(new ArrayList<>());
