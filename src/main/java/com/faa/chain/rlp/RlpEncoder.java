...
import static com.faa.chain.rlp.RlpDecoder.OFFSET_SHORT_LIST;
import static com.faa.chain.rlp.RlpDecoder.OFFSET_SHORT_STRING;
...
public class RlpEncoder {
    ...
    public static byte[] encode(com.faa.chain.rlp.RlpType value) {
        if (value instanceof com.faa.chain.rlp.RlpString) {
            return encodeString((com.faa.chain.rlp.RlpString) value);
        } else {
            return encodeList((com.faa.chain.rlp.RlpList) value);
        }
    }
    ...
    static byte[] encodeList(RlpList value) {
        List<com.faa.chain.rlp.RlpType> values = value.getValues();
        if (values.isEmpty()) {
